...
export default class ChartExtractor extends React.Component<ChartExtractorProps, ChartExtractorState> {
    ...
    extractDataPoints(): CurveData {
        const xPoints: number[] = [];
        const yPoints: number[] = [];

        // Extract from series data
        for (let i = 0; i < this.state.dataTable.series.length; i++) {
            const serie = this.state.dataTable.series[i];
            for (let j = 0; j < serie.shapes.length; j++) {
                const shape = serie.shapes[j];

                let xValue: number;
                let yValue: number;

                // Handle different shape types
                if (shape.textX !== undefined && shape.textY !== undefined) {
                    xValue = this.state.dataTable.axisX.pixelToTick(shape.textX);
                    yValue = this.state.dataTable.axisY.pixelToTick(shape.textY);
                } else if (shape.bbox && shape.bbox.x !== undefined && shape.bbox.y !== undefined) {
                    const centerX = shape.bbox.x + shape.bbox.width / 2;
                    const centerY = shape.bbox.y + shape.bbox.height / 2;
                    xValue = this.state.dataTable.axisX.pixelToTick(centerX);
                    yValue = this.state.dataTable.axisY.pixelToTick(centerY);
                } else if (shape.path && shape.path.length > 0) {
                    // Extract from path commands
                    let foundCoords = false;
                    for (const pathCmd of shape.path) {
                        const type = pathCmd[0];
                        const args = pathCmd[1];

                        if (type === ShapeCommand.types.MOVETO || type === ShapeCommand.types.LINETO) {
                            if (args && args.length >= 2) {
                                xValue = this.state.dataTable.axisX.pixelToTick(args[0]);
                                yValue = this.state.dataTable.axisY.pixelToTick(args[1]);
                                foundCoords = true;
                                break;
                            }
                        }
    ...
    render(): JSX.Element {
        ...
        //     { i: 'axisTableX', x: 0, y: 1, w: 6, h: 3 },
        //     { i: 'axisTableY', x: 0, y: 4, w: 6, h: 3 },
        //     { i: 'dataTable', x: 0, y: 6, w: 6, h: 8 },
        //     { i: 'newChart', x: 6, y: 1, w: 8, h: chartViewHeightUnits },
        //     { i: 'chartView', x: 14, y: 1, w: 9, h: chartViewHeightUnits },
        //     { i: 'statusBar', x: 0, y: chartViewHeightUnits + 1, w: 24, h: 1, static: true },
        // ];
        const layout = [
            { i: 'menuBar', x: 0, y: 0, w: 24, h: 1, static: true },

            // First: Chart View (main chart display)
            { i: 'chartView', x: 0, y: 1, w: 8, h: chartViewHeightUnits },

            // Second: Axis and Data Tables (grouped together)
            { i: 'axisTableX', x: 8, y: 1, w: 5, h: 3 },
            { i: 'axisTableY', x: 8, y: 4, w: 5, h: 3 },
            { i: 'dataTable', x: 8, y: 7, w: 5, h: chartViewHeightUnits - 6 },

            // Third: New Chart (reconstructed chart)
            { i: 'newChart', x: 13, y: 1, w: 5, h: chartViewHeightUnits },

            // Fourth: Curve Display (curve fitting panel)
            { i: 'curveDisplay', x: 18, y: 1, w: 6, h: chartViewHeightUnits },
                    ...
                    <div key="menuBar" className="title">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ color: '#242424' }}>
                                <i className="fa fa-bar-chart" aria-hidden="true"></i>  Chart Simulator
                            </div>

                            <div></div>
                        </div>
                    </div>
                    <div key="curveDisplay" className="panel">
                        <div className="panelDragHandle panelTitle">
                            <i className="fa fa-line-chart" aria-hidden="true"></i> Individual Curve Fitting
                        </div>
                        <div style={{ flex: 1, width: "100%", height: "calc(100% - 30px)", overflow: "auto" }}>
                            <CurveDisplay state={this.state} />
                        </div>
                    </div>
